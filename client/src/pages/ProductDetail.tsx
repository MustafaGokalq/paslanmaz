...
const ProductDetail: React.FC = () => {
  ...
  return (
    <div className=' my-2 flex flex-col gap-y-8 '>
        {/* TOP SİDE */}
        <div className=' border border-black w-full h-full md:h-[580px] rounded-xl shadow-lg p-8 flex flex-col md:flex-row gap-10'>
            {/*SUB IMAGES */}
            <div className=' flex flex-row gap-x-5 md:gap-x-0 md:flex-col justify-between gap-y-4 h-full cursor-pointer '>
                {
                    subImages.map((item,i) => {
                        return(
                            <div onClick={() => handleClickSubImages(i)} key={i} className={`${activeImage == item.img ? 'border-2 border-darkDanger' : 'border border-black'} w-40 h-32 md:h-40 flex justify-center shadow-lg items-center bg-secondary rounded-xl`}>
            ...
            <div className=' w-full flex justify-center flex-col gap-y-5 '>
                {/* TTTLE */}
                <div className='flex w-full justify-between px-0 md:px-72 '>
        ...
        <div>
            {/* DESCRİPTİON */}
            <div className=' border rounded-xl border-black h-full md:h-[450px] bg-secondary shadow-lg p-4'>
                <p>
